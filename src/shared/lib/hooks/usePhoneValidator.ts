export const usePhoneValidator = () => {
   const phoneValidator = (value: string) => {
      ...
      if (cvLength < 11) return `${codeCountry} (${codeCity}) ${firstBlockNumbers}-${secondBlockNumbers}`;
   ...
   const checkValidate = (value: string) => {
      ...
      else if (value.length !== 18) error = 'Неправильный формат телефона. Ожидается: +7 (999) 999-99-99';
